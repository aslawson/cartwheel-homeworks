...
def create_session(body: SessionCreate) -> dict[str, Any]:
    ...
    if user.role != body.role:
        raise HTTPException(
            status_code=403,
            detail=f"user {user.id} is '{user.role}', not '{body.role}'",
        )

    # Built from the stored row, never from the request body: the caller does
    # not get to choose their role or their store.
...
async def post_message(
    ...
    capture = os.environ.get("TRACELOOP_TRACE_CONTENT", "false").strip().lower() in {
        "true",
        "1",
        "yes",
    }
    ...
    with _tracer.start_as_current_span("cartwheel.session_message") as span:
        ...
        # Inside the span: attributes written after it ends are dropped.
        if capture:
            span.set_attribute(
